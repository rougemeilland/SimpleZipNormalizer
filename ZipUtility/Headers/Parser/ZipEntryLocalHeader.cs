...
namespace ZipUtility.Headers.Parser
{
         ...
         : ZipEntryInternalHeader
    {
            ...
            ValidationStringency stringency)
        {
            ...
            var (localHeaderPosition, minimumHeaderBytes, fullNameBytes, extraFieldsBytes) = ReadHeader(zipStream.Stream);
        ...
        private static (ZipStreamPosition localHeaderPosition, ReadOnlyMemory<Byte> minimumHeaderBytes, ReadOnlyMemory<Byte> entryFullNameBytes, ReadOnlyMemory<Byte> extraFieldsBytes) ReadHeader(IZipInputStream zipStream)
        {
            ...
            try
            {
                ...
                if (extraFieldsBytes.Length != extraFieldsLength)
                    throw new BadZipFileFormatException($"Unable to read local header to the end.: position=\"{localHeaderPosition}\"");
                return (localHeaderPosition, minimumHeaderBytes, fullNameBytes, extraFieldsBytes);
            }
