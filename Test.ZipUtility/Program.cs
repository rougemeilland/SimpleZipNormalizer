...
namespace Test.ZipUtility
{
    internal class Program
    {
        ...
        {
            var entryEncodingProvider = ZipEntryNameEncodingProvider.CreateInstance();
            var zipArchive = baseDirectory.GetFile($"{fileName}.zip");
            using (var zipWriter = zipArchive.CreateAsZipFile(entryEncodingProvider))
            {
            ...
            var result = zipArchive.ValidateAsZipFile(entryEncodingProvider);
